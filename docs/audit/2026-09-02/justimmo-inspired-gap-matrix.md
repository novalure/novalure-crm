...
| Release evidence | Git-clean SHA-256 inventory covers migrations 080–084 and all rollback companions. | Checksums identify exact LF-canonical candidate bytes and the inventory has its own SHA-256 sidecar. | Candidate commit is not frozen, evidence is untracked/unsigned, migrations are not executed, and no final release attestation exists. |
