...
3. Contact- und Deal-Creator registrieren den Hauptdatensatz sowie Consent bzw. Stage-History atomar in `qa_batch_objects`, sobald der explizite Preview-QA-Vertrag aktiv ist. `/api/admin/qa-batch-capability` bestätigt denselben Kandidaten-SHA erst nach Prüfung von Cookie-Session, Launch-Scope/RBAC, Allowlist, `workspaces.is_qa` und Ledger-Verfügbarkeit. PATCH/Archiv dürfen nur bereits batchzugehörige Hauptobjekte ändern.
   ...
   Die lokale Simulation beider Startreihenfolgen ist grün. Der echte `--execute`-Lauf gegen den isolierten Preview-Batch bestand vier von vier Reihenfolgen mit zwei getrennten gepinnten `novalure_app`-Sessions, bestätigten Rollbacks und `writes: 0`. Der erste Lauf deckte einen ACL-Konflikt auf: `FOR SHARE` auf der append-only Batchzeile verlangte das absichtlich nicht erteilte UPDATE-Recht. Der Runtimepfad lockt deshalb nur noch den mutablen QA-Workspace; der exklusive Batch-Advisory-Lock bleibt die Serialisierungsgrenze. Ein unabhängiger Owner-read-only-Postcheck bestätigte QA-Flag und Batch sowie null Objekt-, Audit- und Markerreste.
...
| `npm.cmd run test:unit` | PASS, 241/241 Basissuite plus 255/255 Remediation = 496/496 |
...
| `node --test scripts/qa-batch-lock-order-live-tests.mjs` | PASS, 9/9; Production-/Target-Deny, transaction-gepinnte zwei Sessions, gültige unterschiedliche PIDs, bestätigte Rollbacks samt Negativfall, append-only-ACL-konformer Workspace-Lock, Reset-/Mutation- und QA-Flag-Races, redigierter Fehlerpfad und offline Default |
| `npm.cmd run qa:batch-lock-order:execute -- --workspace-id <uuid> --batch-id <uuid> --actor-id <uuid>` | PASS; isolierte Preview-Main, 4/4 Barrieren beobachtet, zwei getrennte Sessions, `writes: 0`; unabhängiger Postcheck: QA=true, Batch vorhanden, 0 Batchobjekte, 0 Reset-Audits, 0 Markerreste |
...
| read-only Zwei-Tenant-Preflight auf Preview-SHA `2a765eb` | PASS, 94/94 Ergebnisse und 50 Requests; zehn konfigurierte Rollen-/Reset-Clients mit Session 200, Rollen-/Sessionbindung, Tenant-Isolation, öffentliche Grenzen, App-Rolle/RLS/Ledger-Projektion und DB-Gates belegt; null CRM-Geschäftswrites, zehn akzeptierte Logout-303. Wegen nachfolgendem Lock-/ACL-Fix auf dem neuen SHA-identischen Kandidaten zu wiederholen |
