...
- dem auf Preview-SHA `b8c72e3745c42aaceab0ee013cfe5bef117e47a3` bestandenen read-only Auth-/Tenant-/DB-Preflight sowie zwei gemäß Stop-Contract beim ersten Kontakt-POST mit 503 abgebrochenen Full-Execute-Versuchen; deren Cleanup hinterließ jeweils null Live-Rückstand, die vollständige Preview-CRUD-/RBAC-/Cleanup-Matrix sowie Provider-, Browser- und Production-Prüfungen bleiben jedoch offen;
...
- Ein deterministischer Zwei-Tenant-QA-Harness samt Konfigurations-, Target-, Rollen-, Batch-, Secret-Evidence- und Cleanup-Guards ist vorhanden. Zwei isolierte `is_qa=true`-Tenants mit insgesamt neun Auth-Identitäten und zehn MFA-fähigen Mitgliedschaften sind provisioniert; jeder weitere Execute-Lauf erhält frische append-only Batches. Der read-only Auth-/Tenant-/DB-Preflight bestand auf Preview-SHA `b8c72e3745c42aaceab0ee013cfe5bef117e47a3` mit 94/94 Ergebnissen und 50 Requests. Zwei getrennte Full-Execute-Versuche mit jeweils frischen Batches stoppten deterministisch beim ersten `POST /api/crm/contacts` mit 503. Der Harness führte den Cleanup trotzdem für beide Tenants aus; die unabhängigen Postconditions bestätigten jeweils null registrierte oder live verbliebene CRM-Objekte. Die Ursache lag vor der Datenbank: Ein Semikolon in einem SQL-Kommentar der QA-Batch-Verfügbarkeitsabfrage wurde vom Tenant-Single-Statement-Guard als zweite Anweisung interpretiert. Der minimale Kommentarfix und ein Regressionstest über den echten Tenant-Guard sind lokal grün. Eine zusätzliche Probe über das reale Repository mit `novalure_app` persistierte Kontakt und Consent samt committed QA-Batchregistrierung; der anschließende echte Reset löschte exakt beide Zeilen und hinterließ null Live-Rückstand. Dieser Repository-/Reset-Beleg ist kein bestandener deployter Full-E2E. Dafür sind ein neues SHA-identisches Preview des Fix-Commits, erneut 94/94 Preflight und der vollständige HTTP-Matrix-/Cleanup-Lauf weiterhin erforderlich.
...
| <code>npm run test:unit</code> | 497/497 | 241/241 Basissuite plus 256/256 Go-Live-Remediation, zwei Runner; unter Node 24.14.0/npm 11.9.0 wiederholt |
...
| <code>npm run test:go-live-remediation</code> | 256/256 | aktueller Delta-Stand einschließlich Scope, Providergrenzen, Proof-Refresh/Visits, Funnel-Token-CAS, durable Webhooks, Blob, QA-Batch-Ownership, Zwei-Tenant-Vertrag, funktionaler MFA-Challenge-Cookie-Rotation, gepinntem Zwei-Session-Barrieredrill, Migrationsparser und Regression für SQL-Kommentare am Tenant-Single-Statement-Guard |
...
| Booking-Zielsuites | gezielt grün | Create/Cancel/Reschedule Launch-off in API, Repository und UI; im aktuellen Remediation-Lauf 256/256 enthalten |
...
Der aktuelle Delta-Stand ist für Unit 497/497 (241/241 Basis plus 256/256 Remediation), Integration 15/15 und i18n 10/10 grün. Unter der exakt gepinnten Ziel-Toolchain Node 24.14.0/npm 11.9.0 sind Toolchain-Check, vollständiges ESLint, Typecheck, Security Audit mit 0 Vulnerabilities, <code>git diff --check</code> und der Next-Production-Build mit 84 generierten Seiten bestanden. Der Remediation-Verbund enthält jetzt auch die gepinnte PgBouncer-Zwei-Session-Prüfung, den append-only-ACL-konformen Workspace-Lock, fail-closed Rollback-/PID-Negativfälle und den Regressionstest für die QA-Batch-SQL-Abfrage durch den realen Tenant-Single-Statement-Guard. Die fokussierte Zwei-Tenant-Suite bestätigt den funktionalen Cookiefluss einschließlich Rotation und sichere Redirectdiagnostik mit 15/15. Die zwei älteren Funnel-P1-Befunde zu Proof-Refresh und Visit-Persistenz sind auf Kandidatencode-/Contract-Ebene adressiert; 33/33 lokale Zieltests sind grün. Der deployte Browser-/Expiry-/HTTP-/Postgres-Nachweis fehlt weiterhin. Public-Funnel-Visit-Tracking bleibt LAUNCH-OFF und gilt nicht als operativ abgenommen. Ältere Zähler sind supersediert; die unveränderliche Kandidaten-SHA entsteht mit dem anschließenden Commit-Freeze und wird im Deployment-Nachweis festgehalten.
...
Damit sind 34 Remediation-Suites in <code>test:go-live-remediation</code> verdrahtet und 256/256 Tests bestanden. Provisionierung, Code-/Contract-, Barrier- und lokale Repository-/Reset-Evidenz ersetzen noch keinen vollständig bestandenen Runtime-/Cleanup-E2E-Nachweis auf dem SHA-identischen Preview-Kandidaten.
...
| aktuell geprüfter Preview-Kandidat | <code>dpl_5ZgXwxJqZyUUhyE95QPZBVYHUhjk</code>, READY, Source-SHA <code>b8c72e3745c42aaceab0ee013cfe5bef117e47a3</code> | Preview-only; temporärer Zugang deployment-spezifisch freigegeben, Geheimwert nicht dokumentiert; Production unverändert |
| read-only Zwei-Tenant-Preflight | 94/94 Ergebnisse, 50 Requests, zehn authentifizierte Rollen-/Reset-Sessions, acht fremde Tenant-Reads korrekt 403, zwei anonyme API-Reads 401 | valide Teilevidenz auf <code>b8c72e3</code>; keine CRM-Mutationsroute im Preflight |
| Zwei-Tenant-Full-Execute | zwei getrennte Läufe mit frischen Batches stoppten jeweils beim ersten Kontakt-POST mit 503; Cleanup für beide Tenants lief, unabhängiger Live-Rückstand jeweils 0 | deterministischer Runtimefehler auf <code>b8c72e3</code>; vollständige CRUD-/RBAC-/IDOR-Matrix nicht ausgeführt und nicht bestanden |
| Repository-/Reset-Probe | reales Repository mit <code>novalure_app</code> persistierte Kontakt + Consent und bestätigte committed QA-Batchregistrierung; Reset löschte exakt 1 Kontakt + 1 Consent, Live-Rückstand 0 | isolierte Preview-Main, aber kein deployter HTTP-Full-E2E; lokale Fixvalidierung |
...
Es wurden Remediation-Previews ausschließlich auf dem autorisierten Branch deployt; Production-Kandidat, Promotion und Aliasumschaltung wurden nicht ausgeführt. Das Preview auf `b8c72e3` reproduziert den durch ein Kommentarsemikolon ausgelösten Tenant-Single-Statement-Guard-Fehler und ist deshalb nicht der finale SHA-identische Kandidat. Der minimale lokale Fix benötigt einen neuen unveränderlichen Commit und ein neues Preview; Baseline-Deployment und Baseline-Logs dürfen weiterhin nicht als Evidenz für den geänderten Code verwendet werden.
...
Konsequenz: Die frühere Blob-Kollision ist auf Ressourcen-, Connection- und Codeebene remediated. ENV-01 bleibt dennoch <strong>FAIL / FEHLER</strong>, weil Queue-/Providertrennung sowie ein vollständiger SHA-identischer Preview-Runtime-/Uploadnachweis fehlen. Die sichere QA-Tenant-/Batch-Provisionierung, der read-only Auth-/Tenant-/DB-Preflight auf `b8c72e3` und der direkte Preview-Main-Barrieredrill sind erfolgt. Zwei Preview-Full-Execute-Versuche erreichten den ersten Kontakt-POST, stoppten dort jedoch mit 503; Cleanup/Reset der leeren Laufbatches hinterließ null Rückstand. Der vollständige CRM-CRUD-Lauf, Upload und Provider-Smoke wurden nicht bestanden beziehungsweise nicht ausgeführt.
...
|---|---|---|
| REL-01 | FAIL / FEHLER | lokaler Freeze unter Node 24.14.0/npm 11.9.0 für Toolchain, Unit 497/497, Remediation 256/256, Integration 15/15, i18n 10/10, vollständiges ESLint, Typecheck, Security Audit, Diff-Check und Production-Build mit 84 Seiten grün; nach dem Kommentarsemikolon-Fix fehlt der neue SHA-identische Preview-Deploy samt Deployment-/Alias-Parität |
...
| CRUD-01 | FAIL / FEHLER | Migrationen 068–077 sind auf isoliertem Preview angewandt; zwei reale `is_qa`-Tenants, zehn MFA-fähige Mitgliedschaften und branchgebundene Runtime-ENV sind vorhanden. Der Auth-/Tenant-/DB-Preflight auf `b8c72e3` bestand 94/94. Zwei Full-Execute-Versuche stoppten deterministisch beim ersten Kontakt-POST mit 503; der Cleanup hinterließ jeweils null Live-Rückstand. Der Kommentarsemikolon-Root-Cause ist lokal minimal behoben und durch Regression sowie reale Repository-/Reset-Probe mit exakt einem Kontakt und Consent bestätigt. Der vollständige HTTP-Matrix-/Reset-/Cleanup-E2E auf einem neuen SHA-identischen Fix-Preview fehlt weiterhin |
...
| PROD-01 | NOT RUN / NICHT AUSGEFÜHRT | Remediation-Preview `b8c72e3` deployt und READY, aber der Full-Execute reproduzierte den Kontakt-503 und machte den lokalen Kommentarsemikolon-Fix erforderlich; kein finaler SHA-identischer Kandidat unter kanonischem Alias und kein kontrollierter Production-Smoke |
| CLEAN-01 | NOT RUN / NICHT AUSGEFÜHRT | beide fehlgeschlagenen Preview-Execute-Läufe führten den API-Cleanup für beide leeren Laufbatches aus und hinterließen null Live-Rückstand. Die reale Repository-Probe persistierte Kontakt + Consent und der echte Reset löschte exakt beide mit null Live-Rückstand. Der vollständige deployte HTTP-Matrixlauf mit CRM-Objekten, Dry-run-/Execute-Digestabgleich und unabhängiger Gesamtreconciliation fehlt weiterhin |

## 8. Befund → Fix → Test/Evidenz → Reststatus

| Befund | Umgesetzter Fix | Test/Evidenz | Reststatus |
|---|---|---|---|
| Fixture-/Fallback-Risiko bei Forms/Knowledge/Funnel | DB-only-Repositories und fail-closed Resolver; Knowledge approved/search nur mit externem Provider | zielgerichtete Forms-/Knowledge-/Funnel-Suites und Unit 497/497 | DATA-01 für den Code-Contract bestanden; deployed E2E weiterhin offen |
...
| Form-Submit-Teilwrites/instabile Retries/Infoleak | lease-gefenceter atomarer Domain-/Minimalresponse-CTE; semantischer Multipart-Fingerprint; Public-Allowlist-DTO; Owner-Tenant-Guard; shared Identity-Locks/Conflict-Schutz; authentifiziertes Admin-JSON streamingbegrenzt auf 256 KiB | Forms 12/12, Reviewer-Bündel 39/39, Migration 20/20, Remediation 256/256, Migrationen 071/072 auf isoliertem Preview | Kandidatencode geschlossen; deploytes Submit-/Cleanup-E2E offen |
...
8. Zwei QA-Tenants und echte Rollen sind provisioniert; der read-only Auth-/Tenant-/DB-Preflight ist auf `b8c72e3` 94/94 grün und der direkte Preview-Main-Barrieredrill 4/4 grün. Zwei Full-Execute-Versuche stoppten beim ersten Kontakt-POST mit 503 und hinterließen nach Cleanup null Live-Rückstand; der Root-Cause ist lokal behoben und per Repository-/Reset-Probe bestätigt. Die Wiederholung als vollständiger HTTP-E2E auf dem neuen Fix-SHA sowie CRM-CRUD-, RBAC-, IDOR-, Auth-, Datei-, Newsletter-, Data-Hygiene-, Bot-, OAuth- und Integrations-E2Es fehlen.
...
11. Preview `dpl_5ZgXwxJqZyUUhyE95QPZBVYHUhjk` für SHA `b8c72e3745c42aaceab0ee013cfe5bef117e47a3` ist READY und mit 94/94 vorgeprüft, reproduziert aber den Kontakt-503. Für den lokal behobenen Kommentarsemikolon-Root-Cause fehlen das neue SHA-identische Preview, der vollständige grüne Execute, Kandidaten-Screenshots, vollständiges Axe-/Screenreader- und Mobile-Set sowie Lighthouse-/Web-Vitals-Evidenz. Ein Production-Kandidat wurde nicht deployt.
...
- kein Production-CRM-Geschäftsobjekt-Seed; in isolierter Preview-Main persistierte ausschließlich die begrenzte Repository-Probe exakt einen Kontakt plus Consent und entfernte beide anschließend über den sicheren Reset;
- kein vollständiger Preview-CRUD und kein Preview-/Production-Upload; die zwei Preview-HTTP-Execute-Versuche stoppten vor dem ersten CRM-DB-Write;
- kein echter Provider-Mailversand und kein Kalendereintrag;
- kein dauerhafter QA-CRM-Geschäftsobjekt-Rückstand; die begrenzte Repository-Probe wurde mit committed Batchregistrierung geschrieben und vollständig zurückgesetzt;
...
- ausschließlich der kontrollierte Cleanup der Preview-Laufbatches und der Repository-Probe; alle verwendeten Execute-/Probe-Batches sind versiegelt und der nächste Lauf benötigt frische Batches.
...
4. Die zehn bereits branchgebundenen Preview-ENV beibehalten, den Kommentarsemikolon-Fix committen und dessen exakt gepinnten neuen Kandidaten-SHA deployen; Production-ENV und Production-Deployment bleiben gesperrt. Für dessen temporären Zugang ist eine neue, deployment-spezifische Freigabe erforderlich.
