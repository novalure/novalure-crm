...
Es existiert inzwischen eine versionierte, immutable und bei unbekannten Surfaces fail-closed arbeitende Launch-Scope-Policy (`2026-08-22.7`) mit 25 Regeln. Sie ist als technischer Kandidat für die remediated Surfaces in UI, API, Cron, Queue, Provideradapter und Repository verdrahtet, trägt jedoch ausdrücklich `PENDING_SIGNATURE`. Der neue Inbound-Bot-Channel-Guard beendet Webhook-POSTs vor Body, Datenbank, LLM und Providerwirkung. Eine signierte Vollmatrix und die vollständige Abdeckung jeder inventarisierten Produktfläche fehlen weiterhin.
...
| LS-12 | Kein zentraler Abschalter für alle Flächen; **Kandidaten-Teilschluss** für die remediated Surfaces | gemeinsame Policy deckt Webhook, Import, Newsletter/Customer-Communication, Calendar, Booking, Reservation-Sync, Funnel-Token, Public Forms, QA und Diagnostik ab; übrige Produktflächen fehlen | zentrale versionierte Enforcement-Policy auf sämtliche inventarisierte OFF-/INTERNAL-Surfaces erweitern |
...
| Dateien und Medien | innerhalb Properties/Bots/Communication | `/api/media`, `/api/media/[assetId]`, `/api/media/files/[assetId]`, `/api/media/public/[token]` | private Pfade RBAC; Public Share Token; Preview verwendet ausschließlich Preview-Blob-Token und fällt ohne diese fail-closed aus | Bot-Document-/Security-Tests plus Preview-Blob-Isolationscontract | Blob-Ressourcen/Connections getrennt; deployter Upload-/Read-/Delete-/Private-Migration-E2E offen |
...
| Bots | `#bots`, `botGovernance` | `/api/bots/*`, `/api/crm/bots` | Permissions/Capabilities; monotone Runtime-Kill-/Test-/Approval-/Knowledge-Kontrollen; signierter Webhook; Bot-/Dokument-/Meta-Providerwirkung zentral über `customerCommunicationProviderMutation` OFF | Bot-Grounding-/Webhook-/Provider-Boundary-Tests | Bot-Chat-/UI-Productstate und Provider-Smoke offen; Providerwirkung bleibt bis Signatur geschlossen |
...
| QA Reset und atomare QA-Batchregistrierung (nicht sichtbare Adminfunktion) | kein produktiver Nav-Eintrag | `/api/admin/qa-reset`, `/api/admin/qa-batch-capability`, QA-Header auf Contact/Deal | Cookie-Session, zentrale INTERNAL-Policy, Owner+`platform_admin` für Capability/Reset, Preview-only SHA-/Zwei-Tenant-Allowlist, Dry-run, Execute-Env+Bestätigung | QA-Reset-, QA-Batch- und Zwei-Tenant-Contracttests; Migration 068 plus zwei QA-Tenants auf isoliertem Preview; read-only Auth-/Tenant-/DB-Preflight auf `7faeda2` 94/94 | Product/Ops/Security-Signatur, Preflight-Wiederholung auf Nachfolge-SHA, Capability-Proof, vollständige Runtime-Matrix und echtes Cleanup-E2E fehlen |
...
| Resend E-Mail | Newsletter, Meeting Notifications, Bot-/Dokumentversand, Einladungen, Passwortreset, Customer Access | expliziter Purpose im zentralen Adapter; Newsletter separat OFF; Meeting-/Bot-/Dokument-/QA-Testversand über `customerCommunicationProviderMutation` vor Provider- und Send-State OFF; unbekannte Purposes fail-closed; Passwortreset und Workspace-Einladung getrennte Account-Access-Verträge | Domain/From/QA-Mailbox/ENV-Ziel und echte Providerabnahme fehlen; Account-Access-Flows benötigen eigenen deployten E2E-Nachweis |
| Microsoft 365 / Graph / Teams Meetings | Calendar Sync, Booking, OAuth | `calendar:sync`+`calendar:manage`; verschlüsselte Tokens; OAuth State/PKCE; zentrale Calendar-Mutation-Policy ist Launch-off vor Body/DB/Provider | QA-Kalender, Zieltrennung, Providerfreigabe und echter Lifecycle-Smoke offen |
| Google Calendar / Google Meet | Calendar Sync, Booking, OAuth | wie Microsoft; zentrale Calendar-Mutation-Policy ist Launch-off vor Body/DB/Provider | QA-Kalender, Zieltrennung, Providerfreigabe und echter Lifecycle-Smoke offen |
...
Der aktuelle Kandidat ist lokal unter Node 24.14.0/npm 11.9.0 mit 494/494 Unit-/Remediation-Tests (241 Basis + 253 Remediation), Integration 15/15, i18n 10/10, Toolchain-Check, Typecheck, vollständigem ESLint, Security Audit mit 0 Vulnerabilities, `git diff --check` und Next-Production-Build mit 84 generierten Seiten grün. Der Remediation-Verbund wurde nach der MFA-Challenge-Cookie- und Log-Redaction-Nachschärfung nochmals mit 253/253 bestätigt; die fokussierte Zwei-Tenant-Suite ist 15/15 grün. Die unabhängige Schlussprüfung des Harness-Deltas weist `0 P0/0 P1` aus; die beiden früheren Funnel-P1-Befunde zu Proof-Refresh und Visit-Persistenz sind nach 33/33 fokussierten Tests als überholt bestätigt. Der read-only Auth-/Tenant-/DB-Preflight auf dem supersedierten Preview-SHA `7faeda2` bestand 94/94, muss aber auf dem nächsten SHA-identischen Kandidaten wiederholt werden. Dies ist keine Product-Freigabe und keine deployte Scope-Abnahme des neuen Kandidaten.
