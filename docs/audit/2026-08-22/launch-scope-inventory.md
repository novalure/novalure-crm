...
| QA Reset und atomare QA-Batchregistrierung (nicht sichtbare Adminfunktion) | kein produktiver Nav-Eintrag | `/api/admin/qa-reset`, `/api/admin/qa-batch-capability`, QA-Header auf Contact/Deal | Cookie-Session, zentrale INTERNAL-Policy, Owner+`platform_admin` für Capability/Reset, Preview-only SHA-/Zwei-Tenant-Allowlist, Dry-run, Execute-Env+Bestätigung | QA-Reset-, QA-Batch- und Zwei-Tenant-Contracttests; Migration 068 plus zwei QA-Tenants auf isoliertem Preview | Product/Ops/Security-Signatur, SHA-identische Runtime-Matrix und echtes Cleanup-E2E fehlen |
...
Der aktuelle Kandidat ist lokal unter Node 24.14.0/npm 11.9.0 mit 492/492 Unit-/Remediation-Tests (241 Basis + 251 Remediation), Integration 15/15, i18n 10/10, Toolchain-Check, Typecheck, vollständigem ESLint, Security Audit mit 0 Vulnerabilities, `git diff --check` und Next-Production-Build mit 84 generierten Seiten grün. Der Remediation-Verbund wurde nach dem letzten Scope-/Dokument-Delta nochmals mit 251/251 bestätigt. Die unabhängige Schlussprüfung des letzten Deltas weist `0 P0/0 P1` aus; die beiden früheren Funnel-P1-Befunde zu Proof-Refresh und Visit-Persistenz sind nach 33/33 fokussierten Tests als überholt bestätigt. Dies ist keine Product-Freigabe und keine deployte Scope-Abnahme.
