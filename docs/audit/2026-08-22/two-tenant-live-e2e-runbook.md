...
3. Migration `057` sowie die Migrationen `068` bis `076` stehen mit exakt den lokal berechneten SHA-256-Checksummen im Preview-Schema-Ledger; eine bloß formal 64-stellige Fremdchecksumme reicht nicht. Das read-only `pg_catalog`-Gate verlangt für 075 Tabelle, Unique-/FK-/Check-Constraints, Expiry-Index und Minimalgrants sowie für 076 die exakten State-/Event-Spalten, validierten Checks, Workspace-Unique-, Account/Event-, Legacy-Abwesenheits-, Reclaim- und Account/Received-Indexzustände, 7 Event-Unique-Indizes, die globale Envelope-Quarantänetabelle, die write-only `SECURITY DEFINER`-RPC mit `search_path=pg_catalog` und Minimalgrants, 6 tenantqualifizierte FKs und bewusst keinen Live-FK vom append-only `audit_logs`-Snapshot.
...
Das isolierte Preview-Projekt `weathered-term-98273025`, Branch `br-lucky-heart-alrm9dlw`, enthält Migration 060 sowie 068–072 und zwei ausschließlich synthetische `is_qa=true`-Wurzeln. Auf Evidence-Branch `br-spring-math-aljuzher` wurde der vollständige Cutover 057 + 073–076 angewandt, mit 5/5 exakten Checksummen, 19/19 validierten deferrable Tenant-FKs, 0/19 Anti-Join-Verstößen und 21/21 Schemaartefakten geprüft, vom Parent wiederhergestellt und anschließend mit identischem Ergebnis erneut angewandt. Vor dem Live-E2E müssen 057 + 073–076 nach dem finalen Commit zusätzlich auf Preview-Main angewandt und dort durch dasselbe Gate bestätigt sein. Die Credentials und TOTP-Secrets wurden lokal in der gitignorierten Datei `.env.qa-two-tenant.local` mit restriktivem Dateimodus abgelegt und weder protokolliert noch committed.
