...
Aktueller Status: **PREFLIGHT 94/94 BESTANDEN / ZWEI FULL-EXECUTE-VERSUCHE AM ERSTEN KONTAKT-POST MIT 503 GESTOPPT / CLEANUP NULL LIVE-RÜCKSTAND / NO-GO**
...
Auf Preview-Deployment `dpl_5ZgXwxJqZyUUhyE95QPZBVYHUhjk` mit Source-SHA `b8c72e3745c42aaceab0ee013cfe5bef117e47a3` bestand der Harness den Modus `preflight` mit 94/94 Ergebnissen und 50 Requests. Belegt sind beide QA-Tenant-Ziele, App-Rolle/RLS/Ledger-Projektion, 11/11 Migrationschecksummen, 19/19 validierte Tenant-Constraints ohne Anti-Join-Verstoß, zehn authentifizierte Rollen-/Reset-Sessions, acht korrekt verweigerte Cross-Tenant-Core-Reads, zwei anonyme 401-Grenzen, zwei öffentliche 200-Seiten ohne fremde Workspace-ID und zehn akzeptierte Logout-Aufrufe mit Status 303. Eine Post-Logout-Invalidierung wurde nicht geprüft. Die Requestliste des Preflights enthält keine CRM-Mutationsroute; Auth-/Session-/Audit-Sicherheitswrites können dennoch erfolgt sein. Preflight-Artefakt: `artifacts/qa/preview-b8c72e3-preflight-20260823/preflight-two-tenant-e2e.json`; kanonischer SHA-256: `0bdc4c36f35a53e9d7a4f38aff73283ec948e86f5702d38eb6587175064fcadb`.

Zwei danach mit jeweils frischen Batches ausgeführte Full-Execute-Versuche stoppten gemäß Verification-Stop-Contract deterministisch beim ersten `POST /api/crm/contacts` mit HTTP 503. Der erste Lauf wies 102 bestandene und zwei fehlgeschlagene Ergebnisse bei 67 Requests aus; der zweite Lauf bestätigte denselben ersten gebrochenen Boundary. In beiden Fällen lief der `finally`-Cleanup für beide Tenants. Die unabhängigen DB-Postconditions bestätigten null registrierte und null live verbliebene Kontakte, Consents, Deals oder Stage-History; append-only Reset-Audits bleiben erwartungsgemäß erhalten. Evidenz: `artifacts/qa/preview-b8c72e3-execute-20260823`, SHA-256 `98a33ad309c9b79f9901cecebc44d40f9bf36930862ecabcfab2df7f5d9c7fd3`, sowie `artifacts/qa/preview-b8c72e3-retry1-execute-20260823`, SHA-256 `acd1c3300712672f35cc2b2c922892d5dde91442ab50d748b44b55b3ed648112`.

Die reproduzierte Ursache lag vor dem Datenbankwrite: Das Semikolon in einem SQL-Kommentar der QA-Batch-Verfügbarkeitsabfrage wurde vom Tenant-Single-Statement-Guard als zweite SQL-Anweisung gewertet. Der minimale Fix ersetzt nur dieses Kommentarsemikolon. Der ergänzte Regressionstest führt die echte Repository-Abfrage durch den realen Tenant-Guard; lokal sind 497/497 Unit-Tests, darunter 256/256 Remediation-Tests, grün. Eine zusätzliche Probe über das reale Repository als `novalure_app` persistierte exakt einen Kontakt plus Consent und bestätigte die committed QA-Batchregistrierung. Der anschließende echte Reset löschte exakt beide Geschäftszeilen; die append-only zwei Ledgerzeilen und der Reset-Audit blieben erwartungsgemäß erhalten, während der Live-Rückstand null war.

Der bestehende PostgreSQL-Barrierenbeleg bleibt 4/4 grün: `artifacts/qa/preview-lock-barrier-pending-sha-20260823/lock-order-evidence.json`, SHA-256 `81af6d6e2ae8580d41e920bc71330265a85b750b17a18c19f7247f6dd3299666`; Postcondition SHA-256 `a61e3013db0d2328cc0e03ef518b8f3f652f3fbc390eeb0177219a145f54cca5`. Weder die lokalen Tests noch die direkte Repository-/Reset-Probe sind ein bestandener deployter Full-E2E. Erforderlich bleiben ein neues SHA-identisches Preview des Fix-Commits, dessen deployment-spezifische Zugangsgenehmigung, frische Batches, erneut 94/94 Preflight und der vollständige grüne HTTP-Execute einschließlich Capability, CRUD/RBAC/IDOR, Dry-run-/Execute-Digestabgleich und Null-Rest-Cleanup. Post-Logout-Invalidierung sowie die vollständige MFA-/Ablauf-/Rate-Limit-Matrix sind ebenfalls nicht belegt.
...
Reale QA-Wurzeln, MFA-Fixtures, Migrationen und der atomare Runtime-Vertrag sind vorhanden. Der read-only Auth-/Tenant-/DB-Preflight auf Preview-Commit `b8c72e3` bestand 94/94 Ergebnisse und 50 Requests; der direkte PostgreSQL-Barrieredrill bestand 4/4 mit Null-Rückstand. Zwei Full-Execute-Versuche stoppten beim ersten Kontakt-POST mit 503, führten aber den Cleanup für beide Tenants aus und hinterließen null Live-Rückstand. Der Kommentarsemikolon-Root-Cause ist lokal minimal behoben und durch Regression sowie reale Repository-/Reset-Probe bestätigt. Offen bleiben das neue SHA-identische Fix-Preview, dessen erneut deployment-spezifisch freizugebender Zugang, frische Batches, der wiederholte Preflight und der vollständig grüne HTTP-Matrix-/Cleanup-Lauf; Post-Logout-Invalidierung und die vollständige Challenge-Matrix sind ebenfalls nicht bewiesen. Das Gesamtgate bleibt rot.
