...
const target = Object.freeze({ project: "weathered-term-98273025", branch: "br-spring-snow-alupo8u4", databases: ["qa_g24_pr63_20260917_r2", "qa_g24_restore_20260917_r2"], runtime: "g24_qa_20260917_r2" });
...
async function assertTarget(client) {
  ...
  assert.ok(target.databases.includes(row.database), "G24 refuses an unowned database");
  assert.equal(row.project, target.project, "G24 refuses a different project");
  assert.equal(row.branch, target.branch, "G24 refuses a different branch");
}
...
export async function seedG24Fixture(admin) {
  ...
  await transaction(admin, null, async client => {
    ...
    await client.query("insert into workspaces(id,name,operating_model,setup_state) values($1,'SYNTHETIC G24 tenant A','managed_by_novalure','{\"syntheticQa\":true}'),($2,'SYNTHETIC G24 tenant B','managed_by_novalure','{\"syntheticQa\":true}')", [ids.workspace, ids.foreignWorkspace]);
    await client.query("insert into workspace_users(id,workspace_id,name,email,role,product_role) values($1,$4,'SYNTHETIC owner','g24-owner@example.invalid','owner','customer_owner'),($2,$4,'SYNTHETIC agent','g24-agent@example.invalid','agent','project_sales_member'),($3,$5,'SYNTHETIC foreign owner','g24-foreign@example.invalid','owner','customer_owner')", [ids.owner, ids.agent, ids.foreignOwner, ids.workspace, ids.foreignWorkspace]);
    ...
    await client.query("insert into tasks(id,workspace_id,project_id,contact_id,title) values($1,$2,$3,$4,'SYNTHETIC G24 task')", [ids.task, ids.workspace, ids.project, ids.contact]);
    await client.query("insert into property_units(id,workspace_id,project_id,unit_number,status,price_cents) values($1,$2,$3,'SYNTHETIC-G24-01','available',35000000)", [ids.unit, ids.workspace, ids.project]);
    await client.query("insert into audit_logs(id,workspace_id,actor_user_id,project_id,action,entity_type,entity_id,after) values($1,$2,$3,$4,'SYNTHETIC_G24_SEED','contact',$5,'{\"synthetic\":true}')", [ids.audit, ids.workspace, ids.owner, ids.project, ids.contact]);
    await client.query("insert into crm_command_receipts(id,workspace_id,project_id,actor_user_id,operation,resource_id,idempotency_key,request_hash,response,audit_reference,correlation_id,data_classification) values($1,$2,$3,$4,'SYNTHETIC_G24_SEED',$5,$6,$7,'{\"synthetic\":true}',$8,$9,'CUSTOMER_TENANT')", [ids.receipt, ids.workspace, ids.project, ids.owner, ids.contact, randomUUID(), hash("SYNTHETIC G24 fixture command"), ids.audit, randomUUID()]);
  }, { commit: true });
  return Object.freeze(ids);
}

/** Every mutation probe rolls back. Results contain check names, never row payloads or credentials. */
export async function verifyG24Security({ admin, runtime, ids }) {
  const checks = [];
  ...
  const check = async (name, fn) => {
    ...
    catch (error) { throw new Error("G24 check failed: " + name + "; " + (error.code ?? error.message), { cause: error }); }
  };
  ...
  await check("080_to_085_protected_relations_enable_and_force_rls", () => transaction(admin, null, async client => {
...
export async function snapshotG24(admin) {
  return transaction(admin, null, async client => {
    ...
    const snapshot = { format: "g24-qa-snapshot-v1", catalog, ledger, data, counts };
