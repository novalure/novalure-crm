...
export const neon061ProfileId = 'neon-g27-provider-creator-admin-only-061-v1';
export const neon061QaTarget = Object.freeze({ projectId: 'super-block-59791927', branchId: 'br-summer-breeze-awuzinct', databaseName: 'qa_g27_20260923', runtimeRole: 'g27_qa_20260923' });
...
function fail(message) { throw new Error('G27_061: ' + message); }
...
async function attestTarget(client, target, localTest, evidence) {
  ...
  if (!/^g27_owner_[a-f0-9]{8}$/.test(localTest.ownerRole) || localTest.grantorRole !== 'qa_admin' || identity.currentUser !== localTest.ownerRole) fail('local synthetic creator/grantor identity mismatch');
...
export async function applyNeon061Compatibility({ client, sql, target, localTest, executionContext }) {
  ...
  await client.query('savepoint g27_neon_061_compat'); // PostgreSQL rejects calls outside a transaction.
  try {
    ...
    await client.query('release savepoint g27_neon_061_compat');
  ...
  } catch (error) { await client.query('rollback to savepoint g27_neon_061_compat'); await client.query('release savepoint g27_neon_061_compat'); throw error; }
}
