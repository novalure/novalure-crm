...
const target = process.env.PROPERTY_DEFAULT_UNITS_MIGRATION_TARGET || "test";
const envFile = target === "prod" ? ".env.production.local" : ".env.local";

if (target !== "test" && target !== "prod") {
  throw new Error("PROPERTY_DEFAULT_UNITS_MIGRATION_TARGET must be 'test' or 'prod'");
}
...
const migration = "migrations/038_property_default_units.sql";
await sql.query(fs.readFileSync(migration, "utf8"));
console.log(`Applied ${migration}`);
console.log("property default units migration applied");
