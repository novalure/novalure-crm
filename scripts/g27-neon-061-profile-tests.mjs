...
import { applyNeon061Compatibility, renderNeon061Compatibility, neon061SourceChecksum, neon061QaTarget } from './lib/g27-neon-061-compat.mjs';
...
test('G27 rendering changes only the two exact creator-edge predicates and pins historical bytes', () => {
...
test('G27 actual local PostgreSQL provider-edge emulation preserves every 061 invariant', {timeout:240000}, async t=>{
 const db=await startLocalSalesDb(),suffix=randomUUID().replaceAll('-','').slice(0,8),owner='g27_owner_'+suffix,provider='qa_admin';
 let client;
 try{
  ...
  await db.admin.query(`grant novalure_tenant_app to ${db.role} with inherit true,set true,admin false; comment on role novalure_tenant_app is 'novalure-tenant-cutover:synthetic-g27-qa';`);
  ...
  await t.test('creator INHERIT, SET, wrong grantor and other admin-only direct members fail closed',async()=>{
   ...
   const other='g27_other_'+suffix;await db.admin.query(`create role ${other} nologin; grant novalure_tenant_app to ${other} with admin true,inherit false,set false`);try{await deny(/unsafe additional/)}finally{await db.admin.query(`revoke novalure_tenant_app from ${other}`)}
  });
  await t.test('runtime high attributes, owner status, ADMIN and indirect privileged paths are rejected',async()=>{
   ...
   const bridge='g27_bridge_'+suffix;await db.admin.query(`create role ${bridge} nologin; grant ${owner} to ${bridge} with inherit false,set false,admin true; grant ${bridge} to ${db.role} with inherit false,set true`);try{await deny(/ADMIN|privileged/)}finally{await db.admin.query(`revoke ${bridge} from ${db.role}`)}
  ...
  await t.test('additional safe-looking LOGIN with inherited owner privileges is not an exempt creator',async()=>{
   const hidden='g27_hidden_'+suffix;await db.admin.query(`create role ${hidden} login inherit; grant novalure_tenant_app to ${hidden} with admin false,inherit true,set true; grant ${owner} to ${hidden} with admin false,inherit true,set true`);
  ...
  await t.test('original cutover, policy, append-only and bad legacy FK guards still refuse activation',async()=>{
   await db.admin.query("comment on role novalure_tenant_app is null");try{await deny(/role comment/)}finally{await db.admin.query("comment on role novalure_tenant_app is 'novalure-tenant-cutover:synthetic-g27-qa'")}
