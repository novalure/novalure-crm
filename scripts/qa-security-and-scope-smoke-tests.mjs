...
test("webhook route has no unsigned Meta, caller-workspace or replay side-effect path", async () => {
  ...
  assert.match(route, /if \(webhookRecord\.duplicate\)[\s\S]*return jsonWebhookResponse\(correlationId, 200\)/);
  assert.ok(route.indexOf("if (webhookRecord.duplicate)") < route.indexOf("runBotChat({"));
  assert.ok(route.indexOf("if (webhookRecord.duplicate)") < route.indexOf("sendBotChannelReply({"));
