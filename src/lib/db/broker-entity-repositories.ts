    ...
    if (isBuyerLead(input.lead)) {
      ...
      return await upsertBuyerSearchProfile({
        profile: {
          ...
          rooms: input.lead.rooms,
          title: input.lead.intent || "Kaeufer-Suchprofil",
        },
        session: input.session,
      });
    }
...
function buyerSearchProfileParams(
  ...
  const params = [
    ...
    existingId ? normalizeUuid(profile.contactId) : cleanString(profile.title) || "Kaeufer-Suchprofil",
    existingId ? cleanString(profile.title) || "Kaeufer-Suchprofil" : moneyToCents(profile.budgetFrom),
