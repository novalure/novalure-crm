...
function buildDateHref(input: {
  ...
  return `/book/${input.slug}?${params.toString()}`;
}

export async function generateMetadata({ params, searchParams }: BookingPageProps): Promise<Metadata> {
...
export default async function BookingPage({ params, searchParams }: BookingPageProps) {
  ...
  const copy = getPublicBookingPageCopy(language);
  const savedPage = await getPublicMeetingPageSettings(slug);
