...
export async function renderPublicFormPage({
  ...
  const crmStatus = getFormQueryValue(query.crm_status);
  const crmReason = getFormQueryValue(query.crm_reason);
  const requestHeaders = await headers();
  const language = resolvePublicLanguage({
    acceptLanguage: requestHeaders.get("accept-language"),
    requestedLanguage: query.lang,
...
export async function renderUnavailableFormPage({
  query,
  slug,
}: {
...
function UnavailableFormNotice({ copy }: { copy: ReturnType<typeof getFormCommandCenterCopy> }) {
  return (
