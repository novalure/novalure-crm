...
export function OfferWorkflow({ deal, contact, leads, workspaceQuery, onChanged }: { deal: Deal; contact?: Contact; leads: Lead[]; workspaceQuery: string; onChanged?: () => Promise<boolean | void> | boolean | void }) {
  ...
  const financialSnapshotKey = offer ? `${offer.id}:${offer.revision}:${offer.contentDigest}:${offer.version}` : undefined;
  const financialSnapshot = financialSnapshotResult && financialSnapshotResult.key === financialSnapshotKey
    ...
    : undefined;
  const financialStatusLabel = financialSnapshot === undefined ? "Finanzstatus wird geladen" : "Finanzprüfung offen";
  ...
  const run = async (operation: string, payload: Record<string, unknown>) => {
    ...
    try {
      ...
      // Reload persisted state; neither an optimistic status nor the command response
      // substitutes for the current offer and its separately verified financial evidence.
      const refreshed = await csrfFetch(`${endpoint}${workspaceQuery ? "&" : "?"}dealId=${encodeURIComponent(deal.id)}`, { cache: "no-store" });
      const persisted = await refreshed.json();
      if (!refreshed.ok) throw new Error(persisted.code ?? persisted.error ?? "Gespeichert; Angebotsakte konnte nicht erneut geladen werden.");
      pendingCommand.current = null;
      setView(persisted as View); setEditing(false); setReference(""); setReason(""); setMessage("Gespeichert. Kein automatischer E-Mail- oder Vertragsversand.");
      await onChanged?.();
  ...
  return <section className="min-w-0 space-y-4 rounded-xl border border-stone-200 bg-stone-50 p-4" aria-label="Angebotsablauf">
    ...
    {!view ? <p className="text-sm">Angebotsakte wird geladen. Bei einem Fehler bitte die Seite neu laden.</p> : <>
      ...
      {offer && <div className="space-y-1 text-sm"><strong>{labels[offer.status]}</strong><p>Revision {offer.revision} · {authoritativeFinancialSnapshot ? `${exactMoney(authoritativeFinancialSnapshot.snapshot.totals.net)} netto` : financialStatusLabel}</p><p>Nachfassen: {offer.followUpStatus}{offer.followUpAt ? ` · ${new Date(offer.followUpAt).toLocaleString("de-AT")}` : ""}</p>{offer.responseReference && <p className="break-words">Antwortbeleg: {offer.responseReference}</p>}</div>}
