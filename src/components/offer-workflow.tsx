...
export function OfferWorkflow({ deal, contact, leads, workspaceQuery, onChanged }: { deal: Deal; contact?: Contact; leads: Lead[]; workspaceQuery: string; onChanged?: () => Promise<boolean | void> | boolean | void }) {
  ...
  return <section className="min-w-0 space-y-4 rounded-xl border border-stone-200 bg-stone-50 p-4" aria-label="Angebotsablauf">
    ...
    {!view ? <p className="text-sm">Angebotsakte wird geladen. Bei einem Fehler bitte die Seite neu laden.</p> : <>
      ...
      {offer && <div className="space-y-1 text-sm"><strong>{labels[offer.status]}</strong><p>Revision {offer.revision} · {money(offer.totalNetCents)} netto</p><p>Nachfassen: {offer.followUpStatus}{offer.followUpAt ? ` · ${new Date(offer.followUpAt).toLocaleString("de-AT")}` : ""}</p>{offer.responseReference && <p className="break-words">Antwortbeleg: {offer.responseReference}</p>}</div>}
      ...
      {offer && !editing && <>
        <details className="rounded-lg bg-white p-3" open><summary className="min-h-11 cursor-pointer font-semibold">Exakte Fassung · Revision {offer.revision}</summary><article ref={articleRef} className="space-y-3 break-words"><h4 className="text-lg font-semibold">{offer.content.subject}</h4><p>An {offer.content.recipientName} · {offer.content.recipientEmail}</p><ul className="space-y-2">{offer.content.items.map((item, index) => <li key={index}>{item.quantity} × {item.description} · {money(item.unitNetCents)} netto = {money(item.quantity * item.unitNetCents)} netto</li>)}</ul><p className="font-semibold">Gesamtverpflichtung: {money(offer.totalNetCents)} netto</p><p className="whitespace-pre-wrap">{offer.content.terms}</p><p>Gültig bis {new Date(offer.content.validUntil).toLocaleString("de-AT")}</p><p className="break-all text-xs">Inhaltsnachweis: {offer.contentDigest}</p></article></details>
        <div className="flex flex-wrap gap-2"><button type="button" className={button} disabled={busy || !["APPROVED", "QUEUED", "SENT", "ACCEPTED", "REJECTED"].includes(offer.status)} onClick={printOffer}>{offer.status === "DRAFT" ? "Entwurf – Druck erst nach Freigabe" : "Fassung drucken"}</button>{["DRAFT", "APPROVED", "SENT"].includes(offer.status) && <button className={button} disabled={busy} onClick={startRevision}>Neue Revision bearbeiten</button>}</div>
