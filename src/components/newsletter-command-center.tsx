...
type LiveNewsletterActionState = {
  detail?: string | null;
...
type NewsletterCommandCenterText = ReturnType<typeof getNewsletterCommandCenterCopy>;
...
function hasNewsletterOptIn(contact: Contact, optInContactIds: Set<string>) {
  return optInContactIds.has(contact.id) || contact.consent.toLowerCase().includes("newsletter");
}
...
function getNewsletterStatusLabel(text: NewsletterCommandCenterText, status: string) {
...
function createBlockId(type: NewsletterEditorBlock["type"]) {
  return `newsletter-${type}-${new Date().getTime()}-${Math.random().toString(16).slice(2)}`;
}
...
function renderNewsletterBlock(block: NewsletterEditorBlock, text: NewsletterCommandCenterText) {
  if (block.type === "text") {
  ...
  if (block.type === "image") {
    ...
    return `<figure><img src="${escapeHtml(block.imageUrl)}" alt="${escapeHtml(block.alt)}" />${caption}</figure>`;
  }

  if (block.type === "button") {
    return `<p><a href="${escapeHtml(block.url)}">${escapeHtml(block.label)}</a></p>`;
  }

  if (block.type === "property") {
    return `<section><h3>${escapeHtml(block.title)}</h3><p>${escapeHtml(block.location)}</p><p>${escapeHtml(block.price)}</p><p>${escapeHtml(block.detail)}</p><p><a href="${escapeHtml(block.ctaUrl)}">${escapeHtml(block.ctaLabel)}</a></p></section>`;
  }

  if (block.type === "feedback") {
    return `<p><strong>${escapeHtml(block.question)}</strong></p>`;
  }

  if (block.type === "social") {
    return `<p><a href="${escapeHtml(block.website)}">${escapeHtml(text.socialWebsite)}</a> | <a href="${escapeHtml(block.instagram)}">${escapeHtml(text.socialInstagram)}</a> | <a href="${escapeHtml(block.linkedin)}">${escapeHtml(text.socialLinkedin)}</a></p>`;
  }

  if (block.type === "spacer") {
    const height = block.size === "large" ? 32 : block.size === "small" ? 12 : 20;
...
function renderNewsletterHtml(draft: DraftState, text: NewsletterCommandCenterText) {
  const blocks = draft.contentBlocks.map((block) => renderNewsletterBlock(block, text)).join("");

  return [
    "<main>",
    draft.previewText ? `<p>${escapeHtml(draft.previewText)}</p>` : "",
    blocks,
    `<footer><p>${escapeHtml(text.footerNote)}</p><p>${escapeHtml(text.legalAddress)}</p><p><a href="${newsletterUnsubscribeUrlToken}">${escapeHtml(text.unsubscribeLink)}</a></p></footer>`,
    "</main>",
  ].join("");
}
...
export function NewsletterCommandCenter({
  ...
  const text = getNewsletterCommandCenterCopy(language);
  ...
  const [liveNewsletterAction, setLiveNewsletterAction] = useState<LiveNewsletterActionState>({
    status: "idle",
  });
  ...
  const hasSelectedCampaign = Boolean(selectedCampaign);
  const selectedCampaignAudience = selectedCampaign?.segment?.audience;
  const selectedCampaignProjectId =
    selectedCampaign?.campaign.projectId ?? selectedCampaign?.segment?.projectId;
  const campaignRecipientEmails = (() => {
    if (!hasSelectedCampaign) {
      return [];
    }

    const optInContactIds = new Set(
      consents
        .filter((consent) => consent.channel === "Newsletter" && consent.status === "Opt-in")
        .map((consent) => consent.contactId),
    );
    const suppressedEmails = new Set(suppressions.map((item) => normalizeEmail(item.email)));
    const emails = contacts
      .filter((contact) => !selectedCampaignProjectId || contact.projectId === selectedCampaignProjectId)
      .filter(
        (contact) =>
          !selectedCampaignAudience ||
          selectedCampaignAudience === "Alle" ||
          contact.role === selectedCampaignAudience,
      )
      .filter((contact) => hasNewsletterOptIn(contact, optInContactIds))
      .map((contact) => normalizeEmail(contact.email))
      .filter((email) => Boolean(email) && !suppressedEmails.has(email));
  ...
  const checklist = [
    ...
    },
    {
      ...
      ready: Boolean(selectedCampaign),
    },
    {
...
const liveNewsletterMessage =
    liveNewsletterAction.detail ??
    (liveNewsletterAction.status === "success"
      ? text.liveSendSuccess
      : liveNewsletterAction.status === "error"
        ? text.liveSendError
  ...
  const sendSelectedNewsletterCampaign = async () => {
    setLiveNewsletterAction({ status: "running" });

    try {
      if (!selectedCampaign) {
        throw new Error(text.noCampaigns);
      }

      if (!campaignRecipientEmails.length) {
      ...
      const response = await fetch("/api/newsletter/send", {
        body: JSON.stringify({
          ...
          from: selectedDraft.fromEmail
            ? `${selectedDraft.fromName || "Novalure CRM"} <${selectedDraft.fromEmail}>`
            : undefined,
          html: renderNewsletterHtml(selectedDraft, text),
          recipients: campaignRecipientEmails,
          replyTo: selectedDraft.replyTo,
          subject: selectedDraft.subject || text.liveSendSubject,
        }),
        headers: { "Content-Type": "application/json" },
        method: "POST",
      });
      const data = (await response.json()) as LiveNewsletterResponse;
      const failedSend = data.sends?.find((send) => send.status === "failed" || send.error);
      const sentCount = data.sends?.filter((send) => send.status === "sent").length ?? 0;

      if (!response.ok || !data.ok || failedSend) {
        throw new Error(failedSend?.error || text.liveSendError);
      }
      ...
      setLiveNewsletterAction({
        detail: error instanceof Error ? error.message : text.liveSendError,
        status: "error",
      });
    }
  };
  ...
  const addSnippet = (snippet: "project" | "viewing" | "seller" | "feedback") => {
    const snippets: Record<typeof snippet, NewsletterEditorBlock[]> = {
      ...
      project: [
        {
          ...
          text: text.projectSnippetHeading,
          type: "heading",
        },
        {
          ctaLabel: text.projectSnippetCta,
          ctaUrl: "{{booking.link}}",
          detail: text.propertyDetailPlaceholder,
          id: createBlockId("property"),
      ...
      seller: [
        {
          id: createBlockId("heading"),
          text: text.sellerSnippetHeading,
          type: "heading",
        },
        {
          id: createBlockId("text"),
          text: text.sellerSnippetText,
          type: "text",
        },
        {
          id: createBlockId("button"),
          label: text.sellerSnippetCta,
          type: "button",
          url: "{{booking.link}}",
        },
      ],
      viewing: [
        {
          id: createBlockId("text"),
          text: text.viewingSnippetText,
          type: "text",
        },
        {
          id: createBlockId("button"),
          label: text.viewingSnippetCta,
          type: "button",
          url: "{{booking.link}}",
        },
      ],
  ...
  function renderBuilderBlockEditor(block: NewsletterEditorBlock, index: number) {
    return (
      <div className="rounded-lg border border-stone-200 bg-white p-4 shadow-sm" key={block.id}>
        <div className="flex items-center justify-between gap-3">
          <p className="min-w-0 break-words text-xs font-semibold uppercase tracking-[0.12em] text-stone-500">
            {index + 1}. {getBlockLabel(block, text)}
          </p>
          <button
            className="shrink-0 rounded-md border border-stone-300 bg-white px-2 py-1 text-xs font-semibold text-stone-700 hover:border-red-200 hover:bg-red-50 hover:text-red-800"
        ...
        {block.type === "text" ? (
          <textarea
            className="mt-3 min-h-36 w-full rounded-md border border-stone-300 bg-white px-3 py-2 text-sm font-medium text-slate-900 outline-none focus:border-slate-950"
          ...
          <div className="mt-3 grid gap-3">
            ...
            <MediaLibraryPicker
              ...
              onSelect={(asset) =>
                updateBlock(block.id, {
                  ...block,
                  alt: block.alt || asset.alt || asset.name,
                  imageUrl: asset.url,
                })
              }
            />
            ...
            <div
              ...
              className="min-h-44 rounded-md border border-dashed border-stone-300 bg-stone-50 bg-cover bg-center p-4 text-sm font-semibold text-stone-500"
              role="img"
          ...
          <div className="mt-3 grid gap-3">
            <div className="grid gap-3 md:grid-cols-2">
              <label className="grid gap-1 text-sm font-semibold text-slate-900">
                ...
                <input
                  className="rounded-md border border-stone-300 bg-white px-3 py-2 text-sm font-medium text-slate-900 outline-none focus:border-slate-950"
                  onChange={(event) => updateBlock(block.id, { ...block, title: event.target.value })}
            ...
            <div className="grid gap-3 md:grid-cols-2">
              <label className="grid gap-1 text-sm font-semibold text-slate-900">
                ...
                <input
                  className="rounded-md border border-stone-300 bg-white px-3 py-2 text-sm font-medium text-slate-900 outline-none focus:border-slate-950"
                  onChange={(event) => updateBlock(block.id, { ...block, price: event.target.value })}
              ...
              <label className="grid gap-1 text-sm font-semibold text-slate-900">
                ...
                <input
                  className="rounded-md border border-stone-300 bg-white px-3 py-2 text-sm font-medium text-slate-900 outline-none focus:border-slate-950"
                  onChange={(event) => updateBlock(block.id, { ...block, ctaUrl: event.target.value })}
                  placeholder={text.buttonUrlPlaceholder}
                  value={block.ctaUrl}
                />
              </label>
            ...
            <label className="grid gap-1 text-sm font-semibold text-slate-900">
              {text.propertyDetail}
              <textarea
                className="min-h-24 rounded-md border border-stone-300 bg-white px-3 py-2 text-sm font-medium text-slate-900 outline-none focus:border-slate-950"
                onChange={(event) => updateBlock(block.id, { ...block, detail: event.target.value })}
        ...
        {block.type === "feedback" ? (
          <label className="mt-3 grid gap-1 text-sm font-semibold text-slate-900">
            ...
            <input
              className="rounded-md border border-stone-300 bg-white px-3 py-2 text-sm font-medium text-slate-900 outline-none focus:border-slate-950"
              onChange={(event) => updateBlock(block.id, { ...block, question: event.target.value })}
        ...
        {block.type === "social" ? (
          <div className="mt-3 grid gap-3">
            <label className="grid gap-1 text-sm font-semibold text-slate-900">
              ...
              <input
                className="rounded-md border border-stone-300 bg-white px-3 py-2 text-sm font-medium text-slate-900 outline-none focus:border-slate-950"
                onChange={(event) => updateBlock(block.id, { ...block, website: event.target.value })}
                placeholder={text.buttonUrlPlaceholder}
                value={block.website}
              />
            </label>
            ...
            <label className="grid gap-1 text-sm font-semibold text-slate-900">
              ...
              <input
                className="rounded-md border border-stone-300 bg-white px-3 py-2 text-sm font-medium text-slate-900 outline-none focus:border-slate-950"
                onChange={(event) => updateBlock(block.id, { ...block, linkedin: event.target.value })}
                placeholder={text.buttonUrlPlaceholder}
        ...
        {block.type === "divider" ? <div className="mt-4 border-t border-stone-300" /> : null}
      </div>
    );
  }

  function renderNewsletterPreviewPanel() {
    return (
      <>
        <article className="rounded-lg border border-stone-200 bg-white p-5">
              ...
              {[
                ...
                <button
                  className={`rounded px-3 py-1.5 ${
                    previewDevice === item.id ? "bg-slate-950 text-white" : "text-stone-600 hover:bg-white"
                  }`}
                  key={item.id}
          ...
          <div
            className={`mt-3 rounded-lg border p-3 ${
              previewIsDark ? "border-slate-700 bg-slate-950" : "border-stone-200 bg-stone-100"
            }`}
          >
            <div
              className={`rounded-md border p-3 shadow-sm ${
                previewIsDark ? "border-slate-700 bg-slate-900" : "border-stone-200 bg-white"
              }`}
            >
              <p className="text-xs font-semibold uppercase tracking-[0.12em] text-stone-500">
                {text.inboxPreview}
              </p>
              <div className={`mt-2 grid gap-1 text-xs ${previewIsDark ? "text-slate-300" : "text-stone-600"}`}>
                <p className="break-words">
                  <span className={`font-semibold ${previewIsDark ? "text-white" : "text-slate-900"}`}>
                    {text.fromLine}:
                  </span>{" "}
                ...
                <p className={`break-words font-semibold ${previewIsDark ? "text-white" : "text-slate-950"}`}>
            ...
            >
              ...
              <div className="px-5 py-5">
                <span className="text-xs font-semibold text-emerald-700">{text.browserVersion}</span>
                <h4
                    ...
                    <div key={block.id}>
                      ...
                      {block.type === "image" ? (
                        <div>
                          ...
                          {block.caption ? (
                            <p className="mt-2 break-words text-xs text-stone-500">{block.caption}</p>
                          ) : null}
                        </div>
                      ) : null}
                      ...
                      {block.type === "property" ? (
                        <div
                          className={`rounded-lg border p-4 ${
                            previewIsDark ? "border-slate-700 bg-slate-800" : "border-stone-200 bg-stone-50"
                          }`}
                        >
                          <p className={`break-words text-lg font-semibold ${previewIsDark ? "text-white" : "text-slate-950"}`}>
                          ...
                          <p className={`mt-2 break-words text-sm ${previewIsDark ? "text-slate-300" : "text-stone-600"}`}>
                            {block.detail || text.propertyDetailPlaceholder}
                          </p>
                          <div className="mt-3 flex flex-wrap items-center justify-between gap-3">
                            <span
                              className={`rounded-md px-3 py-2 text-sm font-semibold ${
                                previewIsDark ? "bg-slate-700 text-white" : "bg-white text-slate-950"
                              }`}
                            >
                      ...
                      {block.type === "feedback" ? (
                        ...
                        >
                          <p className={`break-words text-sm font-semibold ${previewIsDark ? "text-white" : "text-slate-950"}`}>
                      ...
                      {block.type === "spacer" ? (
                        <div
                          className={
                            block.size === "large" ? "h-10" : block.size === "small" ? "h-3" : "h-6"
                          }
                        />
                      ) : null}

                      {block.type === "divider" ? (
                        <div className={previewIsDark ? "border-t border-slate-700" : "border-t border-stone-200"} />
                      ) : null}
                    </div>
                  ))}
                </div>
              ...
              >
                ...
                <div className={`mt-3 flex flex-wrap gap-3 font-semibold ${previewIsDark ? "text-slate-200" : "text-slate-700"}`}>
                  <a className="underline underline-offset-2" href={unsubscribePreviewHref}>
  ...
  if (isBuilderMode) {
    return (
      <section className="fixed inset-0 z-50 flex min-h-0 flex-col bg-[#eef7ff] text-slate-950">
          ...
          <div className="flex min-w-0 flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex min-w-0 items-start gap-3">
              <button
                ...
                onClick={() => setActiveArea("campaigns")}
                type="button"
              >
                {text.backToNewsletter}
              </button>
              <div className="min-w-0">
                ...
                <p className="mt-1 break-words text-xs text-stone-600">
                  {selectedCampaign?.project?.name ?? projectLabel} · {selectedCampaign?.segment?.name ?? text.noSegment}
                </p>
              </div>
            </div>
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
              <p
                className={`break-words rounded-md border px-3 py-2 text-sm font-semibold ${
                  liveNewsletterAction.status === "error"
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-3 py-3 xl:overflow-hidden">
          <div className="grid min-h-0 gap-3 lg:grid-cols-[260px_minmax(0,1fr)] xl:h-full xl:grid-cols-[280px_minmax(0,1fr)_420px] 2xl:grid-cols-[300px_minmax(0,1fr)_500px]">
            <aside className="grid min-w-0 content-start gap-4 rounded-lg border border-stone-200 bg-white p-4 xl:min-h-0 xl:overflow-y-auto">
              <div>
                <p className="text-sm font-semibold">{text.campaignLibrary}</p>
                  ...
                  {filteredCampaigns.map((item) => {
                    ...
                    return (
                      <button
                        ...
                        type="button"
                      >
                        <span className="block break-words font-semibold">{item.campaign.name}</span>
                        <span className={`mt-1 block break-words text-xs ${isSelected ? "text-slate-300" : "text-stone-500"}`}>
                          {item.segment?.name ?? text.noSegment} · {getNewsletterStatusLabel(text, item.campaign.status)}
                        </span>
                      </button>
                    );
                  })}
                  {filteredCampaigns.length ? null : (
                    <p className="rounded-md bg-stone-50 p-3 text-sm font-semibold text-stone-500">
                      {text.noCampaigns}
                    </p>
                  )}
                </div>
              ...
              <div>
                <p className="text-sm font-semibold">{text.blockLibrary}</p>
                <div className="mt-3 grid grid-cols-2 gap-2">
                  {blockActions.map((item) => (
                    <button
                      className="rounded-md border border-stone-300 bg-white px-2 py-2 text-xs font-semibold text-slate-900 hover:border-emerald-200 hover:bg-emerald-50"
                      key={item.label}
                      onClick={item.action}
                      type="button"
                    >
              ...
              <div>
                <p className="text-sm font-semibold">{text.snippets}</p>
                <div className="mt-3 grid gap-2">
                  {snippetActions.map((item) => (
                    <button
                      className="rounded-md border border-stone-300 bg-stone-50 px-3 py-2 text-left text-sm font-semibold text-slate-900 hover:border-emerald-200 hover:bg-emerald-50"
                      key={item.label}
                      onClick={item.action}
                      type="button"
                    >
            ...
            <section className="grid min-w-0 content-start gap-4 rounded-lg border border-stone-200 bg-stone-50 p-4 xl:min-h-0 xl:overflow-y-auto">
              {selectedCampaign ? (
                <>
                  <article className="rounded-lg border border-stone-200 bg-white p-5">
                    <p className="text-sm font-semibold text-slate-950">{text.emailSettings}</p>
                    <div className="mt-3 grid gap-3 md:grid-cols-2">
                      {[
                        { field: "fromName" as const, label: text.fromName, value: selectedDraft.fromName },
                        { field: "fromEmail" as const, label: text.fromEmail, value: selectedDraft.fromEmail },
                        { field: "replyTo" as const, label: text.replyTo, value: selectedDraft.replyTo },
                        { field: "sendAt" as const, label: text.sendAt, value: selectedDraft.sendAt },
                      ].map((input) => (
                        <label className="grid gap-1 text-sm font-semibold text-slate-900" key={input.field}>
                          {input.label}
                          <input
                            className="rounded-md border border-stone-300 bg-white px-3 py-2 text-sm font-medium text-slate-900 outline-none focus:border-slate-950"
                            onChange={(event) => updateDraft(input.field, event.target.value)}
                            value={input.value}
                          />
                        </label>
                      ))}
                  ...
                  <article className="rounded-lg border border-stone-200 bg-white p-5">
                    <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                      <div>
                        <p className="text-sm font-semibold text-slate-950">{text.editingCanvas}</p>
                        <p className="mt-1 text-xs text-stone-500">{text.imageHint}</p>
                      </div>
                      <span className="rounded-md bg-stone-100 px-3 py-2 text-xs font-semibold text-stone-600">
                        {selectedDraft.contentBlocks.length} · {text.contentBlocks}
                      </span>
                    </div>
                    <div className="mt-4 grid gap-3">
                      {selectedDraft.contentBlocks.map(renderBuilderBlockEditor)}
                    </div>
                  </article>
                ...
                <article className="rounded-lg border border-stone-200 bg-white p-5 text-sm font-semibold text-stone-600">
                  {text.noCampaigns}
                </article>
              )}
            </section>

            <aside className="grid min-w-0 content-start gap-4 lg:col-span-2 xl:col-span-1 xl:min-h-0 xl:overflow-y-auto">
              <p className="sr-only">{text.previewAndChecks}</p>
              {renderNewsletterPreviewPanel()}
            </aside>
          </div>
        </div>
      </section>
  ...
  return (
    <section className="grid gap-4">
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div className="min-w-0">
            ...
            <p className="mt-2 max-w-3xl break-words text-sm text-emerald-900">
              {text.liveIntegrationDescription}
            </p>
            <p className="mt-2 break-words text-xs font-semibold text-emerald-900">
              {text.liveSendRecipients.replace(
                "{{count}}",
                formatNumber(campaignRecipientEmails.length, language),
              )}
            </p>
            <p
                  ...
                  return (
                    ...
                    >
                      <span className="flex min-w-0 items-start justify-between gap-3">
                        ...
                        >
                          {getNewsletterStatusLabel(text, item.campaign.status)}
                        </span>
                      </span>
                      <span className="mt-3 grid gap-2 text-xs md:grid-cols-4">
                        ...
                        >
                          {item.segment ? getNewsletterStatusLabel(text, item.segment.health) : text.noSegment}
                        </span>
                        <span
                          className={`rounded-md px-2 py-1 font-semibold ${
                            isSelected ? "bg-white/10 text-white" : "bg-white text-stone-700"
          ...
          <aside className="rounded-lg border border-stone-200 bg-white p-5">
            ...
            <div className="mt-4 grid gap-3 text-sm">
              {[
                ...
                [text.status, selectedCampaign ? getNewsletterStatusLabel(text, selectedCampaign.campaign.status) : undefined],
              ].map(([label, value]) => (
            ...
            <div className="mt-4 rounded-lg border border-emerald-100 bg-emerald-50 p-4 text-emerald-950">
              ...
              <p className="mt-2 text-sm">
                {formatNumber(leads.length, language)} {text.filteredLeads}
              </p>
            </div>
          </aside>
        </section>
      ...
      {activeArea === ("builder" as NewsletterArea) ? (
            ...
            <article className="rounded-lg border border-stone-200 bg-white p-5" key={segment.id}>
              <div className="flex items-start justify-between gap-3">
                ...
                <span className={`rounded-md border px-2 py-1 text-xs font-semibold ${segmentHealthStyles[segment.health]}`}>
                  {getNewsletterStatusLabel(text, segment.health)}
                </span>
              </div>
              ...
              <article className="rounded-lg border border-stone-200 bg-white p-5" key={automation.id}>
                <div className="flex items-start justify-between gap-3">
                  ...
                  <span className={`rounded-md border px-2 py-1 text-xs font-semibold ${automationStyles[automation.status]}`}>
                    {getNewsletterStatusLabel(text, automation.status)}
                  </span>
                </div>
            ...
            <article className="rounded-lg border border-stone-200 bg-white p-5" key={sender.id}>
              <div className="flex items-start justify-between gap-3">
                ...
                <span className={`rounded-md border px-2 py-1 text-xs font-semibold ${deliverabilityStyles[sender.status]}`}>
                  {getNewsletterStatusLabel(text, sender.status)}
                </span>
              </div>
              ...
              <div className="mt-4 rounded-lg bg-slate-950 p-4 text-white">
                ...
                <div className="mt-3 grid gap-2 text-xs text-slate-200">
                  ...
                  <span>unsubscribe · {unsubscribePreviewHref}</span>
                </div>
              </div>
            </article>
          ))}
